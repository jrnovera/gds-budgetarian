  ...
  useEffect(() => {
    ...
    if (state?.orderId) {
      ...
      setTimeout(() => {
        ...
        // Scroll to the order
        const orderElement = document.getElementById(`order-${state.orderId}`);
        if (orderElement) {
        ...
        <div className="p-6 border-b border-yellow-100">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
              <Package className="w-5 h-5 text-red-500" /> All Orders
            </h2>
                            ...
                            .map((history, index) => (
                              <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg border-l-4 border-indigo-400">
                                <div className="flex-shrink-0 mt-1">
                                  {history.status === 'pending' && <Clock className="w-5 h-5 text-yellow-600" />}
                                  {history.status === 'processing' && <Package className="w-5 h-5 text-blue-600" />}
                                  {history.status === 'shipped' && <Truck className="w-5 h-5 text-purple-600" />}
                                  {history.status === 'delivered' && <CheckCircle className="w-5 h-5 text-green-600" />}
                                  {history.status === 'cancelled' && <AlertCircle className="w-5 h-5 text-red-600" />}
                                </div>
